...
function App() {
  ...
  useEffect(() => {
    const fetchSongOfTheDay = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/songs/songoftheday'); // Adjust API endpoint as needed
  ...
  const handleSearch = async () => {
    ...
    switch (searchType) {
      case "all":
        url = "/api/songs/getall";
        break;
      case "artist":
        url = `/api/songs/search?artist=${searchInput}`;
        break;
      case "top5":
        url = "/api/songs/top5";
        break;
      case "details":
        url = `/api/songs/get/${searchInput}`;
        break;
      default:
